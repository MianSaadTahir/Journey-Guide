...
{
    ifstream fileread;
    fileread.open("login.txt"); // Make sure the path is correct for the new file location
...
{
    ofstream filewrite;
    filewrite.open("login.txt", ios::app); // Ensure the path matches
...
{
    ofstream filewrite;
    filewrite.open("ticket.txt", ios::app); // Ensure the path matches
    for (int i = 0; i < no; i++)
    {
